...
class DashboardViewModel: NSObject {
    ...
    func GetLoginData(vc:UIViewController, completion:@escaping (Bool?)->()){

        let header = ["Accept":"application/json",
                "Authorization":UserDefaults.standard.value(forKey: "Ktoken")as! String]
        
        getData(url: KMainUrl+KGetLoginData, parameter: nil, header: header, vc: vc, showHud: "yes", title: "", message: "") { (DictResponce) in
            // print(DictResponce)
    ...
    func GetPostFeedData(tab:Int,Refresh:Bool ,vc:UIViewController, completion:@escaping (Bool?,String)->()){
   
        let arrApis = ["",KfeedsTrending+"?page=1",KfeedsRecent+"?page=1",KfeedsSpotlight+"?page=1",KfeedsPopular+"?page=1"]
...
"Authorization":UserDefaults.standard.value(forKey: "Ktoken")as! String]
    ...
    func PostSlug(Index:Int)->String{
        guard selectedArr != nil else{
            return ""
        }
        guard Index < selectedArr!.count else {
    ...
    func GetPost_MyPhotos(tab:Int,Refresh:Bool ,vc:UIViewController, completion:@escaping (Bool?,String)->()){
      
        let arrApis = ["",KMy_Photos+"?page=1",KMy_Photos+"?page=1",KMy_Photos+"?page=1",KMy_Photos+"?page=1"]
    ...
    func typeOfPost_PHOTOS(Index:Int)->[[String:Any]]?{
        var arrPostMultimediaContent = [[String:Any]]()
        guard selectedArr != nil else {
            return arrPostMultimediaContent
        }
 ...
 // my videos
    ...
    func GetPost_MyVideos(tab:Int,Refresh:Bool ,vc:UIViewController, completion:@escaping (Bool?,String)->()){
    ...
    func typeOfPost_VIDEO(Index:Int)->[[String:Any]]?{
        var arrPostMultimediaContent = [[String:Any]]()
        guard selectedArr != nil else {
            return arrPostMultimediaContent
        }
