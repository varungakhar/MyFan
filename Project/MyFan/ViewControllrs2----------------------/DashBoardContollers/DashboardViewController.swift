...
//        }
    ...
    @IBAction func btnCommentCell(_ sender: UIButton) {
        print(sender.tag)
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "CommentScreenVC")as! CommentScreenVC
        vc.objDashboard2 = objDashboard
    ...
    @IBAction func btnTabActions(_ sender: UIButton) {
        if sender.tag == 1{
         ...
         //   btnTab1.setImage(<#T##image: UIImage?##UIImage?#>, for: .normal)
            ...
            //notification
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "NotificationScreenVC")as! NotificationScreenVC
            self.navigationController?.pushViewController(vc, animated: false)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
         if tableView != tblViewDashBoard {
            ...
            if indexPath.row == 1{
                ...
                // my funds
                let vc = self.storyboard?.instantiateViewController(withIdentifier: "MyFundMainScreen")as! MyFundMainScreen
                self.navigationController?.pushViewController(vc, animated: true)
            }
            else if indexPath.row == 4{
               // my buzz
                let vc = self.storyboard?.instantiateViewController(withIdentifier: "MyBuzzScreenVC")as! MyBuzzScreenVC
                self.navigationController?.pushViewController(vc, animated: true)
            }
...
extension DashboardViewController:UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if indexPath.row < objLoginData.arrIntrst.count{
            ...
            print("ok")
            let vc = self.storyboard?.instantiateViewController(withIdentifier: "FindYourPassionVC")as! FindYourPassionVC
            self.navigationController?.pushViewController(vc, animated: true)
      }
    }
...
extension DashboardViewController{
    func openActionSheet(Index:Int){
        ...
        // hide Post
        let hide = UIAlertAction(title: "Hide Post", style: .default) { (UIAlertAction) in
            
        }
        if let icon = UIImage(named: "ban-2.png")?.imageWithSize(scaledToSize: CGSize(width: 32, height: 32)){
            hide.setValue(icon, forKey: "image")
        }
        
        // Save Post
        let Save = UIAlertAction(title: "Save Post", style: .default) { (UIAlertAction) in
            
        }
        if let icon = UIImage(named: "faved_blue.png")?.imageWithSize(scaledToSize: CGSize(width: 32, height: 32)){
            Save.setValue(icon, forKey: "image")
        }
        ...
        if let icon = UIImage(named: "cancel.png")?.imageWithSize(scaledToSize: CGSize(width: 32, height: 32)){
            Unfollow.setValue(icon, forKey: "image")
        }
