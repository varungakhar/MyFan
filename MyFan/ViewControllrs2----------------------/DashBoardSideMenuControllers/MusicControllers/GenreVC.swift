...
class GenreVC: UIViewController {
    ...
    let objGenreViewModel = GenreViewModel()
    override func viewDidLoad() {
        ...
        let lastItemIndex = IndexPath(item: arr5.count-1, section: 0)
        clcView5.scrollToItem(at: lastItemIndex, at: .right, animated: true)
    ...
    // MARK:- Api Handling
    func getGenreList(){
        objGenreViewModel.GetGenre(vc: self) { (responce) in
...
extension GenreVC:UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tblView.dequeueReusableCell(withIdentifier: "cellid")as! GenreTblCell
       cell.setDataInObj(obj2: objGenreViewModel)
...
extension GenreVC:UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
...
// MARK:- Table cell class  Genre
class GenreTblCell: UITableViewCell,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    
    
    @IBOutlet weak var clcViewMain: UICollectionView!
    
    @IBOutlet weak var heightContrintsClcMain: NSLayoutConstraint!
    let Kheight:CGFloat = 215
    var obj = GenreViewModel()
    ...
    func setDataInObj(obj2:GenreViewModel){
        obj = obj2
         heightContrintsClcMain.constant = (CGFloat((obj.GenreCount()/2))*Kheight)+Kheight
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = clcViewMain.dequeueReusableCell(withReuseIdentifier: "cellid", for: indexPath)as! GenreClcCell
        cell.imgViewBnner.kf.indicatorType = .activity
        cell.imgViewBnner.kf.setImage(with: URL(string: obj.GenreBannerImage(Index: indexPath.row)))
        cell.lblGenreType.text = obj.GenretTitle(Index: indexPath.row)
        cell.imgStationType.kf.indicatorType = .activity
        cell.imgStationType.kf.setImage(with: URL(string: obj.GenreProfileImage(Index: indexPath.row)))
         return cell
    }
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        let GenereID = obj.GenreID(Index: indexPath.row)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        return CGSize(width: collectionView.frame.size.width/2.2, height: Kheight)
    }
    
}
class GenreClcCell:UICollectionViewCell{
